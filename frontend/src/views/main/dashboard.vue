...
const fetchReport = async () => {
    ...
    try {
        ...
        transactions.value = report.data?.data?.data ?? report.data?.data ?? report.data ?? []
    } catch (err) {
...
const dailySeriesFallback = computed(() => {
    ...
    transactions.value.forEach((t) => {
        ...
        if (!totals[day]) {
            totals[day] = { income: 0, expenses: 0 }
        }
...
const paymentMethodLabel = (method) => {
    // El medio de pago puede venir como string simple o como objeto/relación
    // (p. ej. { id, name }). Se soportan ambos casos.
    if (method && typeof method === 'object') {
        return method.name ?? method.label ?? method.description ?? (method.id ?? 'Otro')
    }
...
const paymentMethodPalette = [ '#198754', '#0d6efd', '#6700A6','#EDC000']

const paymentMethodSummary = computed(() => {
    if (byPaymentMethod.value.length) {
        return byPaymentMethod.value.map((row) => ({
    ...
    // cada transacción puede tener varios pagos en transactionPayments, y
    // cada pago su propio medio de pago. Si esa relación no viene cargada,
    // se usa el monto total de la transacción como un único pago
    // (compatibilidad hacia atrás).
    const totals = {}
    const methodKey = (method) => {
        if (method && typeof method === 'object') {
            return method.id ?? method.name ?? 'unknown'
        }
        return method ?? 'unknown'
    }

    transactions.value.forEach((t) => {
        const payments = Array.isArray(t.transactionPayments) && t.transactionPayments.length
            ? t.transactionPayments
            : [{ payment_method: t.payment_method, amount: t.amount }]
    ...
    return Object.values(totals).map((v) => ({ method: v.raw, amount: v.amount }))
})

const paymentMethodChartData = computed(() => ({
    labels: paymentMethodSummary.value.map((row) => paymentMethodLabel(row.method)),
    datasets: [
        {
            data: paymentMethodSummary.value.map((row) => row.amount),
            backgroundColor: paymentMethodSummary.value.map((_, i) => paymentMethodPalette[i % paymentMethodPalette.length]),
        },
    ],
}))
...
// aparezca dentro del periodo consultado.
const methodKey = (method) => {
    if (method && typeof method === 'object') {
        return method.id ?? method.name ?? 'unknown'
    }
